...
interface EvaluationRating {
  ...
  feesCollection: number | null;
  acceptanceFeedback: number | null;
  attitudeJob: number | null;
  attitudeCrowd: number | null;
  assignmentGuidelines: number | null;
  radioCodes: number | null;
  backupCalls: number | null;
  crowdControl: number | null;
  verbalAltercations: number | null;
...
interface GuidedAssignmentData {
  traineeName: string;
  traineeBadge: string;
...
export default function GuidedAssignment() {
  const [formData, setFormData] = useState<GuidedAssignmentData>({
    traineeName: "",
    traineeBadge: "",
    instructorName: "",
    instructorBadge: "",
    ...
    ratings: {
      ...
      attitudeJob: null,
      attitudeCrowd: null,
      assignmentGuidelines: null,
      radioCodes: null,
      backupCalls: null,
      crowdControl: null,
      verbalAltercations: null,
      withColleagues: null,
      withCitizens: null,
...
[hr]
[b][size=150][centre]TRAINEES GUIDED ASSIGNMENT PROGRAM[/centre][/size][/b]
[centre][b][color=#BF0000][sup][b]ASSIGNMENT PERFORMANCE REPORT[/b][/sup][/centre]
...
[centre][divbox=#BF0000][b][color=#FFFFFF]ASSIGNMENT EVALUATION[/color][/b][/divbox][/centre]
...
[/tr]
[tr]
[td]1. Common Sense[/td]${getRatingSymbol(formData.ratings.commonSense)}
[/tr]
[tr]
[td]2. Communication Skills[/td]${getRatingSymbol(formData.ratings.communicationSkills)}
[/tr]
[tr]
[td]3. Fees Collection[/td]${getRatingSymbol(formData.ratings.feesCollection)}
[/tr]
...
[/tr]
[tr]
[td]4. Acceptance of Feedback[/td]${getRatingSymbol(formData.ratings.acceptanceFeedback)}
[/tr]
[tr]
[td]5. Attitude towards the Job[/td]${getRatingSymbol(formData.ratings.attitudeJob)}
[/tr]
[tr]
[td]6. Attitude towards the Crowd[/td]${getRatingSymbol(formData.ratings.attitudeCrowd)}
[/tr]
...
[/tr]
[tr]
[td]7. Familiarization with Assignment guidelines[/td]${getRatingSymbol(formData.ratings.assignmentGuidelines)}
[/tr]
[tr]
[td]8. Department Radio Codes Usage[/td]${getRatingSymbol(formData.ratings.radioCodes)}
[/tr]
...
[/tr]
[tr]
[td]9. Response on Backup Calls[/td]${getRatingSymbol(formData.ratings.backupCalls)}
[/tr]
[tr]
[td]10. Crowd Control[/td]${getRatingSymbol(formData.ratings.crowdControl)}
[/tr]
[tr]
[td]11. Verbal Altercations[/td]${getRatingSymbol(formData.ratings.verbalAltercations)}
[/tr]
...
[size=109][justify]I, ${formData.instructorName}, have completed a Guided Assignment with Trainee Security Officer ${formData.traineeName} on ${formatDateTime(formData.date)}.
I hereby affirm that the aforementioned evaluation scale accurately reflects the performance of the trainee security officer.[/justify][/size]
[/legend]
[/divbox]`;
  };
  ...
  return (
    <div className="w-full max-w-3xl bg-black/40 rounded-xl overflow-hidden border border-white/10 shadow-xl">
      <div className="bg-gradient-to-r from-fuchsia-500/20 to-violet-500/20 border-b border-white/10 p-8">
        <h2 className="text-2xl font-semibold text-white mb-2">Guided Assignment Report</h2>
        <p className="text-sm text-zinc-400 mb-4">
          Document trainee performance during guided assignments.
        </p>
      </div>
      ...
      <form className="p-8 space-y-10">
        ...
        <div className="space-y-6">
          <div className="space-y-1">
            ...
            <p className="text-sm text-zinc-500 ml-10">Record of incidents and tasks during the assignment.</p>
          </div>
              ...
              <div key={index} className="flex gap-4 items-start group">
                <div className="flex-shrink-0 w-24">
                  <div className="flex items-center gap-1">
                    ...
                    <input
                      type="text"
                      value={formData.incidents[index].time || ""}
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-8">
            {/* Entrance Checkpoint */}
            <div className="space-y-4">
              <h4 className="text-sm font-medium text-white/60 uppercase tracking-wider">Entrance Checkpoint</h4>
              <div className="grid gap-4">
                <RatingRow
                  label="Common Sense"
                  field="commonSense"
                  value={formData.ratings.commonSense}
                ...
                <RatingRow
                  ...
                  field="feesCollection"
                  value={formData.ratings.feesCollection}
                  onChange={handleRatingChange}
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-4">
                ...
                />
                <RatingRow
                  label="Attitude towards the Crowd"
                  field="attitudeCrowd"
                  value={formData.ratings.attitudeCrowd}
                  onChange={handleRatingChange}
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-4">
                <RatingRow
                  label="Familiarization with the Assignment guidelines"
                  field="assignmentGuidelines"
                  value={formData.ratings.assignmentGuidelines}
                  onChange={handleRatingChange}
                />
                <RatingRow
                  label="Department Radio Codes Usage"
                  field="radioCodes"
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-4">
                <RatingRow
                  label="Response on Backup Calls"
                  field="backupCalls"
                  value={formData.ratings.backupCalls}
                  onChange={handleRatingChange}
                />
                <RatingRow
                  label="Crowd Control"
                  field="crowdControl"
                  value={formData.ratings.crowdControl}
                  onChange={handleRatingChange}
                />
                <RatingRow
                  label="Verbal Altercations"
                  field="verbalAltercations"
                  value={formData.ratings.verbalAltercations}
                  onChange={handleRatingChange}
                />
              </div>
            </div>
